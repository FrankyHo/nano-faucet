...
module.exports = {
  ...
  async request_light(req, res) {
    ...
    if (Number(addressBalance) >= config.ETH_PAYOUT_VOTERS) {
      return res
        .json({
          error: 'You already have enough ETH!',
          balance: addressBalance,
    ...
    logger(`REQUESTED: ${config.ETH_PAYOUT_VOTERS} ETH from ${address}`);

    try {
      let { transactionHash } = await wallet.sendEther({
        to: address,
        amount: config.ETH_PAYOUT_VOTERS
      });

      let remainingBalance = await wallet.getBalance();

      logger(`SENT: ${config.ETH_PAYOUT_VOTERS} ETH to ${address}`);
      logger(`REMAINING: ${remainingBalance} ETH left in faucet`);
      return res.json({
        transactionHash,
        amount: config.ETH_PAYOUT_VOTERS,
